...
test: compile
	...
	$(CLJE) -pa $(EBIN) -pa $(TEST_EBIN) -e '(require (quote dev.onionpancakes.chassis.tests.test-core) (quote dev.onionpancakes.chassis.tests.test-compiler)) (let [result (clojure.test/run-tests (quote dev.onionpancakes.chassis.tests.test-core) (quote dev.onionpancakes.chassis.tests.test-compiler))] (when (pos? (+ (:fail result) (:error result))) (erlang/halt 1)))'
